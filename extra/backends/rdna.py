...
def emit_vop2(op, dst, src0, src1) -> str:
  # e32 is 32bit repr for the instruction on common cases
  if src0.startswith("v") and src1.startswith("v"):
      return f"{op}_e32 {dst}, {src0}, {src1}"
  else:
      return f"{op}_e64 {dst}, {src0}, {src1}"
...
"""
    ...
    return ".amdgpu_metadata\n" + yaml.dump(metadata) + ".end_amdgpu_metadata" + \
           ...
           '  ' + '\n  '.join(ins) + f"\n.size {name}, .-{name}"
    # put the uops_to_rdna logic here but with the new asm_for_op when no register allocation is needed
